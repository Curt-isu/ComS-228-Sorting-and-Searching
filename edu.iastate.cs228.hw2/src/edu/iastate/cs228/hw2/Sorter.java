...
public abstract class Sorter {
	...
	public void sortWithStatistics(WordList toSort, Comparator<String> comp, int totalToSort) throws NullPointerException, IllegalArgumentException {
		...
		if (toSort == null || comp == null || toSort.length() == 0) {
			throw new NullPointerException();
		}
		
		WordList copy = null;
		CountingComparator<String> cc = new CountingComparator<String>(comp);
		
		while(totalToSort > totalWordsSorted) {
			//Init
			copy = toSort.clone();
			long time = System.currentTimeMillis();
			//Reset, Sort, and Stop time.
			cc.reset();
			sort(copy, cc);
			time = System.currentTimeMillis() - time;
			//Save Data
			totalWordsSorted += toSort.length();
			totalSortingTime += time;
			totalComparisons += cc.getCount();
		}
		
		try {
			File f = new File(getClass().getName() + ".txt");
	 ...
	 */
	/* already completed DIDNT TOUCHED*/
